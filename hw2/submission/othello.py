...
#     best_val = float("-inf") if white_turn else float("inf")
#     legal_moves = generate_legal_moves(board, white_turn)

#     if not legal_moves:
#         # The game is not over because of check_game_over, so it must be the other
#         # player who has legal moves.  Pass the turn to them and don't decrement
#         # the search depth.
#         return minimax_value(board, not white_turn, search_depth, alpha, beta)

#     for m in legal_moves:
#         new_board = play_move(board, m, white_turn)
#         val = minimax_value(new_board, not white_turn, search_depth-1, alpha, beta)
#         if (white_turn and val > best_val) or (not white_turn and val < best_val):
#             best_val = val

#             # But is this path now too good to be true?  No point in seeing
#             # if we could get even better moves when our opponent would block this
#             if (white_turn and val >= beta) or (not white_turn and val <= alpha):
#                 return best_val

#             if white_turn and best_val > alpha:
#                 alpha = best_val
#             elif not white_turn and best_val < beta:
#                 beta = best_val
#     return best_val
