...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        for eA in actionA.effects:
            for eB in actionB.effects:
                if eA == ~eB:
                    return True
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        while not self._is_leveled:
            for item in goals.copy():
                if item in self.literal_layers[-1]:
                    levelsum += i
                    goals.remove(item)
    ...
    def h_maxlevel(self):
        ...
        for item in self.goal:
            for temp, layer in enumerate(self.literal_layers):
                if item in layer:
